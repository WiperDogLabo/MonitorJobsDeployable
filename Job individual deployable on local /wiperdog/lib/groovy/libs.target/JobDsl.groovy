...
public class JobDsl implements JobDSLService {
	def shell
	def jobfacade
	def loader
	...
	public void setListJobs(List<String> listJobs){
	...
	public boolean processJob(File jobfile) {
		try {
			...
			if(listJobs!= null ) {
				if(listJobs.contains(jobfile.getName())){
		...
		try {
			trgfile.eachLine { aline ->
				...
				if (trg != null) {
					...
					if(listJobs.contains(jobname + ".job")) {
						...
						logger.info("No job found for trigger: " + jobname)
					}
				}
			}
		} catch(MultipleCompilationErrorsException e) {
	...
	//When job is removed from listJobs.cfg,remove all
	//relative data from job manager
	public void removeScheduledJob(String jobname){
		def iter = mapInstancesWaitJob.iterator()
		while(iter.hasNext()){
			def instWaitJob = iter.next()
			if(instWaitJob.getKey().equals(jobname)){
				iter.remove();
			}
		}
		iter = mapJobDefaultSchedule.iterator()
		while(iter.hasNext()){
			def instWaitJob = iter.next()
			if(instWaitJob.getKey().equals(jobname)){
				iter.remove();
		...
		iter = lstTriggerWaitJob.iterator()
		while(iter.hasNext()){
			def trgWaitJob = iter.next()
			if(trgWaitJob["jobName"].equals(jobname)){
				iter.remove();
			}
		
		}
		iter = lstJobWaitJobClass.iterator()
		while(iter.hasNext()){
			def jobWaitJobCls = iter.next()
			if(jobWaitJobCls["jobName"].equals(jobname)){
				iter.remove();
			}
		}
		//Remove job and instance was scheduled
		mapJobListInstances.each {key,value ->
			if (key.equals(jobname)) {
				try {
					// Remove inst job
					value.each {eValue ->
						eValue.each {iKey, iValue ->
							if (iKey == "instancesName") {
								def instJobDetail = jobfacade.getJob(key + "_" + iValue)
								jobfacade.removeJob(instJobDetail)
							}
						}
					}
				} catch (Exception ex) {
					println "ERROR REMOVE SCHEDULE: " + ex
				}
			}
		}
		
		def jobOriDetail = jobfacade.getJob(jobname)
		// Remove job ori
		jobfacade.removeJob(jobOriDetail)
